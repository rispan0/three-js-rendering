...
export class VirtualPanoramaViewerComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  onVirtualItemRender(event: VirtualItemRenderEventDetail): void {
    ...
    // Create the panorama item structure without canvas elements
    element.innerHTML = `
  ...
  /**
   * Load a panorama at the specified index
   * @param index The index of the panorama to load
   */
  private loadPanorama(index: number): void {
    const containerId = `panorama-container-${index}`;

    // Skip if already loaded or loading
    if (this.loadingStatus[containerId] || !this.errorStatus[containerId]) {
      return;
    }
    ...
    try {
      // Create the panorama
      this.panoramaService.createPanorama(containerId, this.panoramaUrls[index])
        .then(() => {
